...
## 7. Key Architectural Assumptions to Audit & Real-Data Prompt Optimization Playbook

Before cutting over to Unilever's live `506,531` images/day stream (`Sales EDGE – MT PC` and `GT/Shikkar`), the engineering team must validate **five domain assumptions** that separate Western supermarket benchmarks (`SKU-110K`) from Indian retail execution, followed by a **3-Stage Prompt Optimization Protocol**:

### A. Five Critical Architectural Checks on Day 1 of Real Unilever Images

| # | Component Under Audit | Why Standalone `RT-DETR-v2` / `SKU-110K` Assumptions Break on Indian Shelves | Verification Metric & Production Fix |
| :- | :--- | :--- | :--- |
| **1** | **`Stage 3`: `1-Class` `RT-DETR-v2` vs. Multi-Class `245-SKU` Collapse** | Training `RT-DETR-v2` with `num_classes = 245` fails (`<68%` mAP) because Hungarian bipartite matching destabilizes across `18` near-identical sister shades (`Lakme`/`Ponds`) and extreme class imbalance. | Keep `RT-DETR-v2` strictly `1-class` (`product_facing`) and slice bays horizontally along `Stage 1` shelf rails (`1280x384` per row, `num_queries = 600`) so `15g` face-wash tubes remain `>42px` tall. |
| **2** | **`Stage 3.5`: Hanging Vertical Sachet Ribbons (`"Ladi"` Strips in `GT/Shikkar`)** | `SKU-110K` is `98%` rigid standing bottles/boxes. In Indian `Kirana` (`Shikkar`), `Clinic Plus Rs 1` and `Sunsilk Rs 2` hang in continuous 12-pouch perforated ribbons (`Ladi`). `RT-DETR-v2` draws 1 box around the whole ribbon (`12x` undercount). | Audit sachet strip recall (`Gate G8`): activate the **Perforation-Pitch Slicer** (`H/W > 3.5` -> slice along horizontal heat-seal seams every `5.5 cm`) and fine-tune `1-class` `RT-DETR-v2` on `500` labeled `Ladi` strips (`~4 hours` on 1 L4 GPU). |
| **3** | **`Stage 3.5`: Shrink-Wrapped Multi-Packs (`Lux 3+1 Free` Horizontal Soap Stacks)** | Bar soaps (`Lux`, `Lifebuoy`, `Dove`) are stacked horizontally inside `4-in-1` plastic promo sleeves. Detectors propose both the outer bundle box and the 4 inner bar boxes (`400%` facing inflation). | Audit **Hierarchical Box Containment (`IoU_containment > 0.85`)**: when the outer box matches a `Multipack` SKU in `AlloyDB`, suppress the 4 inner bar boxes and log `pack_multiplier = 4`. |
| **4** | **`Stage 1 & 3`: 2nd-Row Depth Cap Leakage Under `25 deg` Downward Camera Tilt** | Narrow `1.1m` aisles force reps to shoot lower shelves at a downward tilt, exposing the caps of 2nd-row and 3rd-row bottles behind the front facing (`+8% to +14%` false Share-of-Shelf inflation). | Audit **Shelf-Rail Base Y-Alignment**: require front-facing boxes (`depth_row = 1`) to have `y_max` within `+-4%` of the horizontal shelf strip (`y_rail`). Tag floating rear caps as `depth_row >= 2` and exclude from `Linear SoS (cm)`. |
| **5** | **`Stage 4`: Studio CAD Renders vs. Real In-Store Crop Embeddings in `AlloyDB ScaNN`** | Pure 2D CAD studio packshots yield only `0.69–0.76` cosine similarity against curved, dusty, shrink-wrapped store packs—causing `60%` of crops (instead of `11%`) to cascade into `Gemini` (`latency -> 3.8s`). | **Day-1 Multi-Prototype Seeding:** Run `Gemini 3.8 Pro` teacher labeling on the first `500` real HUL store photos to extract `15` real in-store crop centroids per SKU into `AlloyDB pgvector`, restoring `>= 88%` `ScaNN` fast-path hit rate. |

### B. Prompt Optimization Roadmap (Why We Use Contrastive Visual Prompting Instead of Weight `SFT`)

We deliberately **do not** run Supervised Fine-Tuning (`SFT`) on Gemini model weights right now: HUL introduces new promo sashes (`+20% Extra`, festive editions) every 4–6 weeks, which quickly invalidates static fine-tuned weights. Instead, when real HUL images arrive, we execute three targeted prompt optimizations on the `300`-image `Val` split (`splits/dataset_splits_manifest.json`):

| Phase | Target Stage | Prompt Optimization Technique | Expected KPI Gain & Latency Impact |
| :--- | :--- | :--- | :--- |
| **Step 1 (Day 1)** | **`Stage 4.5 & 5a` (`Sister-Shade Disambiguation` — `9%` of crops)** | **3-Panel Contrastive Strip (`[Ref A \| Store Crop \| Ref B]`, `384x128 px`)**: Stitch the `AlloyDB` reference thumbnail for Top-1 Candidate A on the left, the `3x` zoomed `[0.62H:0.88H]` store crop in the middle, and Top-2 Candidate B on the right. Prompt Gemini to perform direct visual verification rather than zero-shot text recall. | **`+3.5% to +4.8%` Sister-Shade `F2`** (`0.942 -> 0.978+`) with **`0 ms`** added latency (`64` fixed output tokens via `response_schema`). |
| **Step 2 (Day 2)** | **`Stage 5b` (`MT Toker Compliance` & Promo Header OCR — `2%` of crops)** | **Indian Multilingual Promo & MRP Lexicon (`Hindi / Marathi / Tamil / Rs`)**: Inject a compact vernacular mapping table into the `Toker` system prompt so regional danglers (`Rs 20 की छूट`, `3+1 मुफ़्त`, thermal sticker `MRP Rs 145/- Offer Rs 125/-`) normalize deterministically into `TokerComplianceAudit` enums. | **`+6.2%` Vernacular Toker OCR Recall** (`91.8% -> 98.0%`) across regional Tier-2 Modern Trade stores. |
| **Step 3 (Day 3)** | **CI/CD Gate (`shelf-bench gate-check` on `Val` Split)** | **Automated Prompt Sweep (`ThinkingBudget = 0` vs `ThinkingBudget = 64`)**: Benchmark 4 candidate system prompts across the locked `300`-image real-store `Val` split using `shelf-bench gate-check` to lock the prompt with highest `7-Dim HUL F2` and `<= 64` output tokens. | Locks production prompt contract behind the **7-Gate MLOps CI/CD check** before promoting to the `Test` split. |
